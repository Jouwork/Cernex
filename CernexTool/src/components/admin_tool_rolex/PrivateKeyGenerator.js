...
class PrivateKeyGenerator extends Component {
    ...
    constructor(props) {
		...
		this.state = {
          ...
          ownerAddress: null,
          isChecked: false,
          corporativeWallet: null
		}
	  }
	
      _getCorporativeWallet = async () => {
        ...
        if(this.state.isChecked){
            const formData = new FormData();
                ...
                const resFile = await axios({
                    method: "get",
                    url: "http://localhost:8080/api/v1/walletAddress",
                    data: formData,
                    headers: {
                        "Content-Type": "multipart/form-data"
                    },
                })
                this.setState({corporativeWallet: resFile.data}) 
                walletAddressInput.value = resFile.data;
        }else{
  ...
  render() {
    ...
    <div className="input-section">
        ...
        <div className="column">
            ...
            <input type="checkbox" name="topping" value="Hold on corporative wallet" checked={this.state.isChecked} onChange={this._getCorporativeWallet} />
            <div className="input-group">
                <label className="input-label">Wallet Address</label>
                <div className="input-container">
                <input type="string" 
                         placeholder="wallet address" id="walletAddress"
    ...
    <div className="back-button-container">
        ...
        <button href="index.html" className="back-button"  onClick={(event) => {
                                window.location.href = "/" }}>Back</button>
